      ...
      <main>
        <section className="overflow-hidden">
          <div className="mx-auto max-w-7xl px-6 pb-24 pt-24 md:pt-32">
            <div className="max-w-4xl">
              <span className="site-accent-soft inline-flex rounded-full border border-blue-200 px-4 py-2 text-sm font-bold text-blue-700 dark:border-blue-900 dark:text-blue-300">Data • Web • AI</span>
        ...
        {featured && (
          <section className="mx-auto max-w-7xl px-6 pb-20">
            ...
            <Link href={`/blog/${featured.slug}`} className="site-subtle group grid overflow-hidden rounded-[2rem] md:grid-cols-2">
        ...
        <section className="site-subtle border-y border-zinc-200 dark:border-zinc-800"><div className="mx-auto max-w-7xl px-6 py-20"><p className="text-sm font-bold uppercase tracking-widest text-blue-600">Topics</p><div className="mt-6 flex flex-wrap gap-3">{categories.map((category) => <Link key={category} href={`/articles?category=${encodeURIComponent(category)}`} className="site-surface rounded-full border border-zinc-300 px-5 py-3 text-sm font-semibold hover:border-blue-500 hover:text-blue-600 dark:border-zinc-700">{category}</Link>)}</div></div></section>
